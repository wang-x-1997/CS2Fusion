...
image_A_list = prepare_data1(r'D:\Image_Data\IRVI\AUIF Datasets\16x\Test_FLIR/')
image_B_list = prepare_data(r'D:\Image_Data\IRVI\AUIF Datasets\16x\Test_FLIR/')
save_path = r"./Fused/"
...
for i in range(len(image_A_list)):
    S = time.time()
    imgA = load_image_VIS(image_A_list[i])
    imgA_Y = RGB2Y(imgA)
    imgB = load_image_IR(image_B_list[i])

    if not opt.cuda:
        model_E = model_1.to(device)
    ...
    with torch.no_grad():
        ...
        E1, E2, E3, E4, f, F = model_1(imgA_Y, imgB)
        FM, M, mf, W = model_2(E1, E2, E3, E4, f)
